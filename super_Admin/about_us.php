            ...
            <div class="box-body">
                   <div class="row">
                       <div class="col-md-12">
                          ...
                          <?php
                                        $srt1=mysqli_query($con,"select * from tbl_about_us WHERE type='Admin'");
                                        $srtt1=mysqli_fetch_assoc($srt1);
                                         if(isset($_POST['submit']) && !empty($_POST['submit']))
                                         {
                                            
                                            $ques = $_POST['ques'];
                                            $srt=mysqli_query($con,"select * from tbl_about_us WHERE type='Admin' ");
                                            $srtt=mysqli_fetch_assoc($srt);
                                            $num=mysqli_num_rows($srt);
                                            if($num>0)
                                            {
                                                $upd=mysqli_query($con,"update tbl_about_us set ques='$ques' WHERE type='Admin'");
                                                if($upd)
                                                {
                                            ...
                                            {
                                                $query=mysqli_query($con,"INSERT INTO `tbl_about_us`(`ques`,type)VALUES ('$ques','Admin')");
                                                if($query)
                                                {
                                      ...
                                      <form role="form" method="post" id="addressForm" onsubmit="return validateForm()" enctype="multipart/form-data">
                                          <div class="form-group">
                                            <textarea type="text" class="form-control" id="addressInput" oninput="clearErrorMessage()" name="ques" required></textarea>
               ...
               <div class="table-responsive">
                             <table id="example2" class="table table-bordered table-striped ">
                                  ...
                                  <tbody>
                    <?php
                        $sqls=mysqli_query($con,"SELECT * FROM tbl_about_us WHERE type='Admin'");
                        $count=0;
                        ...
                        <!--<td><?php echo  $row['tbl_about_us'];?></td>-->
...
<?php
        ...
        {
            $fdid=$_GET['fdid'];
            $del=mysqli_query($con,"delete from tbl_about_us where about_id='$fdid'");
            if($del)
            {
