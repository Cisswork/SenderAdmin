...
class Cerber_taxi
{
    ...
    function add_user_amount()
    {
        ...
        $date = date('Y-m-d');
        $time = date('h:i A');
        $booking_id=mt_rand(10,99);
        
        $upd3=mysqli_query($con,"SELECT * FROM `tbl_user_lynk_id` WHERE `id`='$card_id'");
        $sel3=mysqli_fetch_assoc($upd3);
        $lynk_name = $sel3['lynk_name'];
        
        $select_d= mysqli_query($con,"SELECT * FROM `user_register` WHERE id = '$user_id'");
        $fetch_d= mysqli_fetch_assoc($select_d);
        $user_wallet= $fetch_d['user_wallet'];
        $new_ammount=$user_wallet+$amount;
        ...
        if($status=='RECEIVED')
        {
            $updt=mysqli_query($con,"update user_register set user_wallet='$new_ammount' where id='$user_id'");
            $data= mysqli_query($con,"INSERT INTO `user_withdraw_request`(`user_id`,`withdraw_credit`,`date`,`status`,card_id,`payment_mode`,`booking_id`,time)
            VALUES ('$user_id','$amount','$date','wallet money added','$card_id','','','$time')");
            //$message1['id']=$insert_id;
